 ...
 *   Solution: x&y = ~(~x | ~y)
 */
int bitAnd(int x, int y) {    //solution: x&y = ~(~x | ~y)
  int ret = ~(~x | ~y);     
  return ret;
}
/* 
 ...
 *   Max ops: 6
 *   Rating: 2
 *   Solution：move the byte we need to lower byte ,and & with 0xFF
 */
 ...
 *   Max ops: 20
 *   Rating: 3 
 *   Solution: The key is to make a Binary sequence that start with n 0,following with 32-n 1  
 */
